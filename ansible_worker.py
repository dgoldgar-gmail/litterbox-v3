...
# Clean the vault password from environment
raw_vault_pass = os.environ.get("ANSIBLE_VAULT_PASSWORD", "")
clean_vault_pass = raw_vault_pass.strip('"').strip("'")
...
def run_job(job_file: Path):
    ...
    ansible_config = RUNNER_DIR / "ansible.cfg"
    inventory_path = RUNNER_DIR / "inventory" / "hosts.yml"

    logger.info(f"ansible_config: {ansible_config}")
    logger.info(f"inventory_path: {inventory_path}")

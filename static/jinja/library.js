...
window.loadFile = async function(path) {
    ...
    try {
        const response = await fetch(`/jinja/get-library-file?path=${encodeURIComponent(path)}`);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("Library files:", library_files);


    if (container && typeof library_files !== 'undefined') {
        container.innerHTML = createTreeHTML(library_files);

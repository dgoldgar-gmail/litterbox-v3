document.addEventListener('DOMContentLoaded', () => {
    ...
    function addMangementButtons(container, hostName) {
        ...
        upgradeButton.className = 'btn btn-sm btn-outline-danger';
    ...
    async function upgradeApplication(event, app_name, hostName, application_config) {
        ...
        fetch(getContainerVersionsUrl)
            ...
            .then(optionsList => {
                // optionsList is now a proper JavaScript array
                console.log("Options list:", optionsList);
                return showUpgradeVersionSelectionModal(optionsList);
            })
            .then(selectedVersion => {
                console.log("Selected version:", selectedVersion);
                const streamUrl = `${upgradeContainerizedAppBaseUrl}?host=${encodeURIComponent(hostName)}&container=${encodeURIComponent(app_name)}&version=${encodeURIComponent(selectedVersion)}`;
